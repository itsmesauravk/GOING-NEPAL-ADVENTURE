...
const createBooking = async (req, res) => {
    try {
        const { fullName, email, phone, address, adventureType, adventureName, adventureSlug, adventureId, bookingDate, extraServices, soloStandard, totalPrice, } = req.body;
        if (!fullName ||
            !email ||
            !phone ||
            !address ||
    ...
    <div style="background-color: white; border-radius: 8px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
        <tr>
          ...
          <td style="padding: 12px; border-bottom: 1px solid #f0f0f0;">${create._id}</td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="padding: 12px; border-bottom: 1px solid #f0f0f0; width: 35%; color: #2980b9; font-weight: bold;">Adventure Name:</td>
           <td style="padding: 12px; border-bottom: 1px solid #f0f0f0;">
            ${adventureName}
            <a href="${process.env.CLIENT_URL_PROD}/${adventureType.toLowerCase()}/${adventureSlug}" style="margin-left: 10px; color: #3498db; text-decoration: none;">View Details</a>
          </td>
        </tr>
        <tr>
      ...
      <div style="text-align: center; margin-top: 25px;">
        <a href="${process.env.CLIENT_URL}/bookings/${create._id}" style="background-color: #3498db; color: white; padding: 12px 25px; text-decoration: none; border-radius: 4px; font-weight: bold; display: inline-block;">Review Booking</a>
      </div>
    </div>
