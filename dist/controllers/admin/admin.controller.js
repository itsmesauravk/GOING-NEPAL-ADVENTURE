...
const generateAccessAndRefreshTokens = async (admin) => {
    try {
        ...
        const refreshToken = jwt.sign({ id: admin._id }, process.env.JWT_REFRESH_TOKEN_SECRET, {
...
const registerAdmin = async (req, res) => {
    try {
        ...
        const registredAdmin = await Admin.findById(admin._id).select("_id fullName email createdAt");
    ...
    try {
        ...
        if (admin.failedLoginAttempts >= 5) {
            ...
            if (lastFailedLoginAt) {
                const diff = Math.abs(currentTime.getTime() - lastFailedLoginAt.getTime());
...
const updateAccessToken = async (req, res) => {
    try {
        ...
        const oldRefreshToken = req.cookies.refreshToken || req.header("Authorization")?.split(" ")[2];
        if (!token) {
            return res.status(StatusCodes.UNAUTHORIZED).json({
                success: false,
                message: "Unauthorized access. Token missing.",
...
const getFullAdminProfile = async (req, res) => {
    try {
...
const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        // Generate token
        const verifyToken = jwt.sign({ id: admin._id }, process.env.JWT_ACCESS_TOKEN_SECRET, { expiresIn: "10m" });
...
const verifyOtp = async (req, res) => {
    ...
    try {
        ...
        const resetToken = jwt.sign({ id: admin._id }, process.env.JWT_ACCESS_TOKEN_SECRET, { expiresIn: "10m" });
...
export { registerAdmin, loginAdmin, logoutAdmin, adminProfile, updateAccessToken, validateToken, getFullAdminProfile, editAdmin, updatePassword, forgotPassword, verifyOtp, resetPassword, };
