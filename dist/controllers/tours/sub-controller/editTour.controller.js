...
const editTour = async (req, res) => {
    try {
        const { tourId, imagesToDelete, thumbnailToDelete, videoToDelete, tripType, ...updateData } = req.body;
        ...
        if (imagesToDeleteArray.length > 0) {
            const remainingImages = existingTour.images.filter((img) => !imagesToDeleteArray.includes(img));
        ...
        if (files.thumbnail) {
            ...
            const uploadedThumbnail = await uploadFile(files.thumbnail[0].path, "tour/thumbnail");
        ...
        if (files.video) {
            ...
            const uploadedVideo = await uploadVideo(files.video[0].path, "tour/videos");
            updateFields.video = uploadedVideo?.secure_url;
        }
        // Handle new images upload
        if (files.images && files.images.length > 0) {
            const newImages = await Promise.all(files.images.map((image) => uploadFile(image.path, "tour/images")));
        ...
        if (tripType) {
            ...
            if (parsedTripType._id !== existingTour.tripTypeId) {
                ...
                await TripsAndTours.findByIdAndUpdate(existingTour.tripTypeId, { totalTours: oldTotalTours - 1 }, { new: true });
                // Update new trip type count
                const newTotalTours = await Tour.countDocuments({
                    tripTypeId: parsedTripType._id,
                });
                await TripsAndTours.findByIdAndUpdate(parsedTripType.id, { totalTours: newTotalTours + 1 }, { new: true });
            }
        }
        ...
        const updatedTour = await Tour.findByIdAndUpdate(tourId, { $set: updateFields }, { new: true, runValidators: true });
        if (!updatedTour) {
