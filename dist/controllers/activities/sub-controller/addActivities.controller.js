...
const createActivity = async (req, res) => {
    try {
        const { title, price, country, location, groupSizeMin, groupSizeMax, bestSeason, overview, serviceIncludes, thingsToKnow, FAQs, } = req.body;
        const groupSize = {
            min: groupSizeMin,
            max: groupSizeMax,
        };
        ...
        const galleryUrls = image
            ? await Promise.all(image.map(async (img) => (await uploadFile(img.path, "/activities/image"))?.secure_url))
            : [];
        // Upload video
